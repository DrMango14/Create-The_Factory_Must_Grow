...
public class LargeSwitchRenderer extends KineticBlockEntityRenderer<LargeSwitchBlockEntity> {
    ...
    @Override
    protected void renderSafe(LargeSwitchBlockEntity be, float partialTicks, PoseStack ms,
            MultiBufferSource bufferSource, int light, int overlay) {
        if (!be.getBlockState().getValue(IS_MAIN_PART)) return;

        BlockState blockState = be.getBlockState();
        VertexConsumer vb = bufferSource.getBuffer(RenderType.solid());

        float yRot = blockState.getValue(HORIZONTAL_FACING).getAxis() == Direction.Axis.Z
                ? Math.abs(blockState.getValue(FACING).toYRot())
                : blockState.getValue(FACING).toYRot() - 180;
    ...
    protected SuperByteBuffer getRotatedModel(LargeSwitchBlockEntity be, BlockState state) {
        if (!state.getValue(IS_MAIN_PART))
            return CachedBuffers.partialFacing(TFMGPartialModels.LARGE_SWITCH_BUFFER, state,
                    state.getValue(FACING).getCounterClockWise());
        return CachedBuffers.partialFacing(TFMGPartialModels.LARGE_SWITCH_SHAFT, state,
                state.getValue(FACING).getCounterClockWise());
    }
}
